...
def process_pdf_sync(file_path, group_name):
    ...
    try:
        ...
        # Extract separators primarily via fast PyMuPDF vector drawings
        for page_idx, page_fitz in enumerate(doc_fitz):
            all_separators = []
            for d in page_fitz.get_drawings():
                r = d["rect"]
                # Table horizontal line or thin rectangle
                if r.width > 400 and r.height <= 5:
                    all_separators.append(r.y0)
            
            # Lazy fallback to pdfplumber only if PyMuPDF didn't find drawing lines
            if not all_separators:
                try:
                    with pdfplumber.open(file_path) as pdf_plumb:
                        if page_idx < len(pdf_plumb.pages):
                    ...
                    logging.debug(f"pdfplumber fallback skipped: {plumb_err}")
            
            all_separators.append(0)
            all_separators.append(page_fitz.rect.height)
            all_separators.sort()
